            ...
            <div class="card" v-for="(item) in listData" :key="item.id" @click="edit($event, item)">
                <div class="card-info">
                    <div class="flex flex-center">
                        ...
                        <div class="flex">
                            <van-tag round type="primary">{{ item.is_return == 1 ? '在使用' : '未使用' }}</van-tag>
                        </div>
                    </div>
                    <div class="content">
                        <span class="mr20">
                            <van-icon name="flag-o" size="15" />楼层<i class="text">{{ item.floor }}</i>
                        </span>
                    </div>
                </div>
...
export default {
    ...
    methods: {
        ...
        edit(event, item) {
            switch(event.type) {
                case 'edit':
                    this.$router.push({ path: '/room/form?id='+item.id});
                    break;
                case 'delete':
                    console.log(item);
                    this.$dialog.confirm({
                    ...
                    break;
                default:
                    this.$router.push({ path: '/room/form', query: { id: item.id,  disabled: true }});
                    break;
            }
        }
    }
